...
class LatticeLSTM(nn.Module):
    ...
    def forward(self, input, skip_input_list, hidden=None):
        ...
        for index in range(batch_size):
            hidden_out = []
            memory_out = []
            input_ = input[index, :, :].unsqueeze(0).transpose(1,0)
            hx_ = hx[index, :].unsqueeze(0)
            cx_ = cx[index, :].unsqueeze(0)
            ...
            for t in id_list:
                (hx_,cx_) = self.rnn(input_[t], input_c_list[t], (hx_, cx_))
                ...
                if skip_input_[t]:
                    ...
                    # pytorch 0.3.0
                    ###########################
                    # word_var = autograd.Variable(torch.LongTensor(skip_input_[t][0]))
                    with torch.no_grad():
                        word_var = autograd.Variable(torch.LongTensor(skip_input_[t][0]))
                    if self.gpu:
                    ...
                    ct = self.word_rnn(word_emb, (hx_, cx_))
                    assert(ct.size(0) == len(skip_input_[t][1]))

...
tf.flags.DEFINE_string("exporter_dir", None, "Directory for save model, please use the HDFS path, expample: hdfs://default/$path")
...
with tf.device(
    ...
    if FLAGS.exporter_dir is not None and is_chief:
        ...
        tensor_info_y_ = tf.saved_model.utils.build_tensor_info(cnn.input_y)
        tensor_info_y = tf.saved_model.utils.build_tensor_info(cnn.scores)
        # 定义方法名和输入输出
        signature_def_map = {
